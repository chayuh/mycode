...
def delfiles(chan):
    chan.send('sudo -i')
    chan.send("\n")
    chan.send("rm -f "+linux_shell_path+" "+linux_path+"/"+log_name+" "+linux_path+"/"+user90)
    chan.send('\n')
    buff = ''
...
def execcmd(chan):
    ...
    chan.send('\n')
    global log_name
    if 'alter' in shell_name:
        log_name=host+".log"
    if 'rollback' in shell_name:
        log_name=host+"_rol.log"
    log_exec=linux_shell_path+" "+ linux_path+" "+user90+">"+linux_path+"/"+log_name
    chan.send(log_exec)
    chan.send('\n')
    buff=''
    while not buff.endswith('# '):
